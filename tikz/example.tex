...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=\scalefactor, thick]
    ...
    \path[draw,black] (0,1) node[left] {y} -- (1,0) node[right] {x};
...
\begin{tikzpicture}
  ...
  \draw[ultra thick,->] (-3,-3) -- (-3,3) node[left] {Template Variety};
  \draw[ultra thick,->] (-3,-3) -- (3,-3) node[below] {Narrative Variety};
  \draw (-2,1) node[above] {Kushman};
  \draw (2,-2.5) node[above] {{\sc Dolphin}};
  \draw (-2,0.5) node[above] {{\sc Verb-395}};
  \draw (2,2) node[above] {{\sc Draw}};
\end{tikzpicture}
